...
int main(){
    ...
    TERMINO::ENTITY player2(1,30,0,0,theme.MAGENTA,true);
    TERMINO::ENTITY line(5,1,0,0,theme.WHITE,true);
    ball.sprite[0]="  ▄█████▄  ";
    ball.sprite[1]="▄█████████▄";
    ball.sprite[2]="███████████";
    ball.sprite[3]="▀█████████▀";
    ...
    player.set(win, TERMINO::POS_BOTTOM, player.pos.x/2, -10);
    player2.set(win, TERMINO::POS_BOTTOM, TERMINO::POS_CENTER, -10);
    player2.set(win, TERMINO::POS_BOTTOM, player2.pos.x+player2.pos.x/2, -10);
    line.set(win, TERMINO::POS_BOTTOM, TERMINO::POS_CENTER, -10+(line.size.h/2));
    ...
    int score=0,score2=0;int life=MAX_LIEF;
    std::string scoreStr, scoreStr2,lifeStr;
    win.updateRowsCols();
    for(int i=0;i<BRICKS_PACKS;i++){
        for(int j=0;j<BRICKS_ROWS;j++){
            for(int k=0;k<BRICKS_COLS;k++){
    ...
    while(!quit){
        if(!win.checkKey('q',input)){
            if(fGame.checkFPS(15)){
                win.clear();
                if(win.isWindowResized()){player.set(win, TERMINO::POS_BOTTOM, TERMINO::POS_CENTER, -10); player.set(win, TERMINO::POS_BOTTOM, player.pos.x/2, -10);  player2.set(win, TERMINO::POS_BOTTOM, TERMINO::POS_CENTER, -10); player2.set(win, TERMINO::POS_BOTTOM, player2.pos.x+player2.pos.x/2, -10); line.set(win, TERMINO::POS_BOTTOM, TERMINO::POS_CENTER, -10+(line.size.h/2));}
                for(int i=0;i<BRICKS_PACKS;i++){
                    for(int j=0;j<BRICKS_ROWS;j++){
                        for(int k=0;k<BRICKS_COLS;k++){
                ...
                player2.print(win, TERMINO::POS_BOTTOM, player2.pos.x, -10);
                ball.print(win,ball.pos.y,ball.pos.x);
                enemy.print(win, TERMINO::POS_BOTTOM, 0);
                line.print(win);
                scoreStr = "Score : "+std::to_string(score);
                scoreStr2 = "Score : "+std::to_string(score2);
                lifeStr = "";
                for(int i=0;i<MAX_LIEF;i++) if(i+1<=life){lifeStr += " ♥ ";}else{lifeStr += " ♡ ";}
                lifeStr = "Life : "+lifeStr;
                TERMINO::styleON(theme.WHITE);
                win.print(scoreStr.c_str(),TERMINO::POS_BOTTOM,TERMINO::POS_CENTER, -5, -win.cols/4);
                win.print(scoreStr2.c_str(),TERMINO::POS_BOTTOM,TERMINO::POS_CENTER, -5, win.cols/4);
                win.print(lifeStr.c_str(),TERMINO::POS_TOP, TERMINO::POS_CENTER,2);
            ...
            if(fBall.checkFPS(60)){
                if(!gameover){
                    ...
                    if(TERMINO::ENTITY::checkCollision(ball,player2,TERMINO::COL_TOP)){ up=true;}
                    if(TERMINO::ENTITY::checkCollision(ball,player2,TERMINO::COL_BOTTOM)){ up=false;}

                    for(int i=0;i<BRICKS_PACKS;i++){
                        for(int j=0;j<BRICKS_ROWS;j++){
                            for(int k=0;k<BRICKS_COLS;k++){
                                if(ball.pos.x >= 0 && ball.pos.x+ball.size.w <= win.cols/2){
                                    ...
                                    if(TERMINO::ENTITY::checkCollision(ball,brick[i][j][k],TERMINO::COL_TOP)){ up=true; brick[i][j][k].die(); score2++;}
                                    if(TERMINO::ENTITY::checkCollision(ball,brick[i][j][k],TERMINO::COL_BOTTOM)){ up=false; brick[i][j][k].die(); score2++;}
                                    if(TERMINO::ENTITY::checkCollision(ball,brick[i][j][k],TERMINO::COL_LEFT)){ left=true; brick[i][j][k].die(); score2++;}
                                    if(TERMINO::ENTITY::checkCollision(ball,brick[i][j][k],TERMINO::COL_RIGHT)){ left=false; brick[i][j][k].die(); score2++;}
                                }
                            }
                        }
                    }
                    if(TERMINO::ENTITY::checkCollision(ball,enemy,TERMINO::COL_TOP)){
                        if(--life < 1){ gameover=true;life=3;score2=0;
                        for(int i=0;i<BRICKS_PACKS;i++){
                            for(int j=0;j<BRICKS_ROWS;j++){
                                for(int k=0;k<BRICKS_COLS;k++){
            ...
            if(!gameover){
                ...
                player2.pos.x = ball.pos.x+(ball.size.w/2)-(player2.size.w/2);
            }
            switch (input)
            {
            case KEY_RIGHT: player2.pos.x += 10; break;
            case KEY_LEFT: player2.pos.x -= 10; break;
            ...
            if(player.pos.x+player.size.w>win.cols/2)player.pos.x=win.cols/2-(player.size.w);
            if(player2.pos.x<win.cols/2)player2.pos.x=win.cols/2;
            if(gameover){ ball.set(win, player.pos.y - ball.size.h, player.pos.x  + (player.size.w/2) - (ball.size.w/2)); }
        }else quit=true; 
    }
}